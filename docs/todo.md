...
- [x] 处理错误响应显示 (Toast 通知)
- [x] 实现全局错误边界 (ErrorBoundary 组件)

### 5.2 性能优化
- [ ] 实现消息虚拟滚动 (大量消息)
- [x] 实现图片懒加载 (LazyImage 组件 + IntersectionObserver)
- [ ] 优化重新渲染
- [ ] 添加请求缓存

### 5.3 用户体验
- [x] 添加加载骨架屏 (ChatSkeleton, SidebarSkeleton)
- [x] 添加操作成功/失败提示 (ToastProvider)
- [x] 实现离线状态提示 (OfflineIndicator + useOnlineStatus)
- [x] 优化移动端体验
  - [x] 创建 useMediaQuery 响应式 Hook
  - [x] 实现响应式侧边栏（移动端覆盖层+自动关闭）
  - [x] 优化消息输入区域（防止 iOS 缩放、触控友好）
  - [x] 优化消息列表和消息项样式
  - [x] 优化聊天容器安全区域适配
  - [x] 添加全局移动端 CSS 优化

### 5.4 安全加固
- [x] 实现请求速率限制 (slowapi 速率限制装饰器)
  - 登录: 10次/分钟
  - 注册: 5次/分钟
  - 聊天消息: 20次/分钟
  - 文件上传: 30次/分钟
  - 会话列表: 60次/分钟
- [x] 添加输入清理 (sanitizer.py 工具模块)
  - HTML转义防护
  - 文件名清理
  - 控制字符过滤
- [x] 实现 XSS 防护 (前端 react-markdown + 后端 HTML 转义)
- [x] 审查敏感数据处理 (Key Vault 存储密钥)
